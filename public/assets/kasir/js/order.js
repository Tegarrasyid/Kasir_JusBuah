...
const OrderPanel = (() => {
  ...
  function getFinalPrice(item) {
    if (item.discount_percent > 0) {
      return Math.round(item.price - (item.price * item.discount_percent / 100));
    }
    return item.price;
  }

  function subtotal() {
    return items.reduce((s, i) => s + getFinalPrice(i) * i.qty, 0);
  }
  function discountAmt() {
    return items.reduce((total, item) => {
      if (item.discount_percent > 0) {
        const disc = item.price * item.discount_percent / 100;
        return total + (disc * item.qty);
      }
      return total;
    }, 0);
  }
  ...
  function renderItems() {
    ...
    container.innerHTML = items.map(item => {
      const finalPrice = getFinalPrice(item); // ✅ TARUH DI SINI
        ...
        <div class="order-item" data-id="${item.id}">
          ...
          <div class="item-details">
            ...
            <input class="item-note-input" id="note-${item.id}"
              placeholder="Catatan" value="${item.note}"
              ...
              ${
                ...
                ? `
                  ...
                  <span style="color:var(--green)">
                    ${formatRp(finalPrice * item.qty)}
                  </span>
                  <small style="color:orange">(${item.discount_percent}%)</small>
                `
  ...
  function renderSummary() {
    ...
    if (btn) {
      btn.disabled = (items.length === 0 || bayar < tot);
    }
    ...
    render();
      ...
      if (bayarInput) {
        bayarInput.addEventListener("input", () => {
          ...
          renderSummary(); // 🔥 penting biar tombol update juga
        });
      }
